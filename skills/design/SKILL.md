...
Independent design review of a tool's output against `shipwright.design`.
...
2. Check against the tokens:
   - **Tiers:** does the tool map its domain verdicts to `shipwright.design.tiers.Severity` via `TierMappable` (not hardcoded colors)? Off-axis states mapped explicitly?
   ...
   - **Banner:** via `shipwright.design.banner.make_banner`, to stderr, suppressed on quiet/pipe.
   - **Formats:** the `--output` set matches the `shipwright.design.output` contract (`rich|console|json|ndjson|csv`); `console` is the reduced-density plain formatter.
