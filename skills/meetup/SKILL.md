...
6. **Skeptic-gate (bounded loop):** dispatch the independent reviewer over the drafted decision (vote honest? follows from positions? claims verified vs the artifact? dissent preserved?). Loop fix → re-review until clean APPROVE, **capped at 5 iterations**. If iteration 5 is still not clean, **STOP and ask the human what to do instead** (accept-and-record residual findings · narrow to the clean part · send back to brainstorming · abandon) — never silently loop past 5 or record an un-APPROVED decision as decided. Record the iteration count + residual findings in the log.
