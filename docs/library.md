...
The `shipwright` library is import-light: `import shipwright` pulls in no `rich` or
...
`import shipwright`. (Distribution name is `shipwright-kit`; see the README.)

---

## `shipwright.design` — severity tiers + accessible output
...
```python
from shipwright.design import Severity
...
```python
from shipwright.design import Severity, tier_label, glyph
...
```python
from shipwright.design import Severity, TierMappable
...
## `shipwright.eval` — detection-quality eval harness
...
```python
from shipwright.eval import Sample, load_corpus
...
```python
from shipwright.eval import Sample, evaluate
...
```python
from shipwright.eval import gate, EvalGateError
gate(r, min_precision=1.0, min_recall=0.9)   # no raise on the result above
```

---

## `shipwright.security` — security pack

Ships with the base install and registers via the `shipwright.packs` entry point
...
```python
from shipwright.design import Severity
from shipwright.security.eval import is_alert, SECURITY_MIN_PRECISION, SECURITY_MIN_RECALL
...
`shipwright.security.theme` maps `Severity` onto security labels (CLEAN → LOW →
